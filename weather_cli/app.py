...
def format_weather_data(
    data: Dict, include_forecast: bool = False, user_id: str = None
) -> str:
...
def get_weather(city: str, include_forecast: bool = False) -> Union[str, Dict]:
    ...
    try:
        ...
        response = session.get(
            url,
            headers={"Host": "api.openweathermap.org", "User-Agent": "Mozilla/5.0"},
            timeout=10,
        )
            ...
            if isinstance(result, dict):
                ...
                if "conditions" in notifications[city]:
                    ...
                    if current_condition in notifications[city]["conditions"]:
                        alerts.append(
                            f"{city}: {current_condition} conditions detected"
                        )
...
def get_weather_map(city: str, map_type: str = "temperature") -> str:
    ...
    try:
        ...
        forecast_data = requests.get(
            forecast_url, headers={"Host": "api.openweathermap.org"}
        ).json()
        ...
        if map_type == "temperature":
            ...
            rains = [
                item["rain"]["3h"] if "rain" in item else 0
...
def get_radar_map(city: str) -> str:
    ...
    try:
        ...
        radar_data = requests.get(
            radar_url, headers={"Host": "api.openweathermap.org"}
        ).json()

        # Create color scale for precipitation
        rains = [
            item["rain"]["3h"] if "rain" in item else 0 for item in radar_data["list"]
        ]
        colormap = cm.LinearColormap(
            ["white", "lightblue", "blue", "darkblue"],
            vmin=0,
        ...
        # Add time slider
        plugins.TimeSliderChoropleth(
            time_slider_data, period="PT3H", name="Radar Animation"  # 3-hour intervals
        ).add_to(m)
...
def main():
    ...
    while True:
        ...
        if choice in ["2", "6", "7", "8", "9", "11", "12", "13"]:
            user_id = input(
                "Enter your user ID (or press Enter to continue as free user): "
            ).strip()
            if not check_premium_access(user_id):
                ...
                upgrade = (
                    input("\nWould you like to upgrade to Premium? (y/n): ")
                    .strip()
                    .lower()
                )
                if upgrade == "y":
        ...
        if choice == "1":
            ...
            try:
                temp_threshold = float(
                    input("Enter temperature threshold (leave empty to skip): ").strip()
                    or 0
                )
                if temp_threshold:
                ...
                conditions = input(
                    "Enter weather conditions to alert (comma-separated, leave empty to skip): "
                ).strip()
                if conditions:
                    notifications[city]["conditions"] = [
                        c.strip().lower() for c in conditions.split(",")
                    ]
            ...
            if not city:
                ...
                continue
            map_type = (
                input("Enter map type (temperature/precipitation/wind): ")
                .strip()
                .lower()
            )
